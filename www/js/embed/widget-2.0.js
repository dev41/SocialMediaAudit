function seoptimerSubmit(el) {
    if (!seoptimerFormValidate(el.id)) {
        //e.preventDefault();
        return false;
    }
    ...
    if (behaviour == 'modal') return seoptimerSubmitModal(el.id);
    if (behaviour == 'be_in_touch') return seoptimerSubmitBeInTouch(el.id);
    if (behaviour == 'redirect') return seoptimerSubmitRedirect(el.id);
}

function seoptimerFormValidate(id) {
...
function seoptimerSubmitModal(id) {
    ...
    html += "\t\t\t\t\t<span onclick=\"closeSeoptimerModal();\" style=\"display:inline-block; width:16px; height:16px; position:absolute; top:15px; right:15px; cursor:pointer; background-image:url('data:image/png;base64,'); opacity:0.6;\"></span>\n" +
            ...
            "\t\t\t</div>\n" +
            "\t\t</div>";
    modalWrapper.setAttribute('style', 'position:absolute; top:0px; left:0; width:100%; z-index:9999999');
    ...
    soBody.classList.add("no-scrollbar");
...
function seoptimerSubmitBeInTouch(id) {
    var form = document.getElementById(id);
...
function seoptimerSubmitRedirect(id) {
    var form = document.getElementById(id);
...
function closeSeoptimerModal() {
    ...
    setTimeout(function () {
        soBody.classList.remove("no-scrollbar");
